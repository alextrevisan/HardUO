...
--MultiTiles-----------------------------------------------
  if #HouseItems > 0 then
    for i=1,#HouseItems do
      if HouseItems[i].State~="uncertain" and IsInside(X,Y,HouseItems[i].Area.X,HouseItems[i].Area.Y,HouseItems[i].Area.X2,HouseItems[i].Area.Y2) then return false end    
    end
  end 
--StaticTiles----------------------------------------------- 
  for i=2,TileCnt do
...
--MultiTiles-----------------------------------------------
  if #HouseItems > 0 then
    for i=1,#HouseItems do
      if HouseItems[i].State~="uncertain" and IsInside(X,Y,HouseItems[i].Area.X,HouseItems[i].Area.Y,HouseItems[i].Area.X2,HouseItems[i].Area.Y2) then print("Stopped by multi!") return false end    
    end
  end  
--StaticTiles-----------------------------------------------  
  for i=2,TileCnt do
...
function CheckMovement(X,Y,Z,Direction,IgnoreDoors,IgnoreSpellFeilds)
  ...
  ---- Note: {Z=StartZ} not {Z+StartTop}... changed to fix head head. will see how this runs
